...
if [ ! -e /var/lib/mysql/ibdata1 ] ; then
  ...
  service mysqld start && mysql -p$(cat /root/mysql_password) --connect-expired-password -e "ALTER USER user() IDENTIFIED BY '${MYSQL_ROOT_PASSWORD}'; CREATE USER root@'%'; ALTER USER root@'%' IDENTIFIED BY '${MYSQL_ROOT_PASSWORD}'; GRANT ALL ON *.* TO root@'%' WITH GRANT OPTION" && service mysqld stop
fi
